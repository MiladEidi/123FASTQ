...
import Eidi._123Fastq.QualityControlPackage.Graphs.KmerLineGraph;
...
public class KmerContent extends AbstractQCModule implements MergeableQCModule {

    private Hashtable<String, Kmer> kmers = new Hashtable<>((int) Math.pow(4, MAX_KMER_SIZE));
...
//		}
    ...
    public boolean ignoreInReport() {
        return ModuleConfig.getParam("kmer", "ignore") > 0;
    }
    ...
    public JPanel getResultsPanel() {
        ...
        if (enrichedKmers.length > 0) {
            ...
            splitPanel.setTopComponent(new KmerLineGraph(enrichments, minGraphValue, maxGraphValue, "Position in read (bp)", xLabels, xCategories, "Log2 Obs/Exp"));
     ...
     * seen at each position within the run. We can use this later on to
     * calculate the enrichment of the Kmers we actually count.
     *
     * We take in the Kmer sequence even though this isn't used in the total
     * counts we do this because we don't want to count Kmers with Ns in them,
     * but we do need to ensure that the data structure is expanded to the right
     * size, and if we have libraries where later positions are Ns in all
     * sequences then our data structure ends up too short and we crash.
     *
...
//		System.err.println("Max value="+maxGraphValue+" min value="+minGraphValue);
    ...
    public void mergeFrom(MergeableQCModule other) {
        KmerContent o = (KmerContent) other;

        if (o.longestSequence > longestSequence) {
            longestSequence = o.longestSequence;
        }

        // Merge the 2-D position-count array (position × kmer-length).
        if (o.totalKmerCounts.length > totalKmerCounts.length) {
            long[][] expanded = new long[o.totalKmerCounts.length][];
            for (int i = 0; i < totalKmerCounts.length; i++) {
                expanded[i] = totalKmerCounts[i];
            }
            for (int i = totalKmerCounts.length; i < expanded.length; i++) {
                expanded[i] = new long[MAX_KMER_SIZE];
            }
            totalKmerCounts = expanded;
        }
        for (int i = 0; i < o.totalKmerCounts.length; i++) {
            for (int j = 0; j < o.totalKmerCounts[i].length; j++) {
                totalKmerCounts[i][j] += o.totalKmerCounts[i][j];
            }
        }

        // Merge the per-kmer observation tables.
        // (Kmer is a private inner class — outer class can access its fields.)
        for (java.util.Enumeration<String> e = o.kmers.keys(); e.hasMoreElements(); ) {
            String key = e.nextElement();
            Kmer incoming = o.kmers.get(key);
            Kmer existing = kmers.get(key);
            if (existing != null) {
                existing.count += incoming.count;
                if (incoming.positions.length > existing.positions.length) {
                    long[] newPos = new long[incoming.positions.length];
                    for (int i = 0; i < existing.positions.length; i++) {
                        newPos[i] = existing.positions[i];
                    }
                    existing.positions = newPos;
                }
                for (int i = 0; i < incoming.positions.length; i++) {
                    existing.positions[i] += incoming.positions[i];
                }
            } else {
                kmers.put(key, incoming);
            }
        }
    ...
    public boolean raisesError() {
        ...
        return enrichedKmers.length > 0 && 0 - Math.log10(enrichedKmers[0].pValue()) > ModuleConfig.getParam("kmer", "error");
    }
    ...
    public boolean raisesWarning() {
        ...
        return enrichedKmers.length > 0 && 0 - Math.log10(enrichedKmers[0].pValue()) > ModuleConfig.getParam("kmer", "warn");
    }

    public void makeReport(HTMLReportArchive report, int No) throws IOException, XMLStreamException {

        //Handle tables in the Comprative-mode
        if (!calculated) {
            calculateEnrichment();
        }

        if (enrichedKmers.length > 0) {
            writeSpecificImage(report, new PercentLineGraph(enrichments, minGraphValue, maxGraphValue, "Position in read (bp)", xLabels, xCategories, "Log2 Obs/Exp"), "kmer_profiles.png", "Kmer graph", Math.max(800, groups.length * 15), 600);
        }
    ...
    private class ResultsTable extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            switch (columnIndex) {
                case 0:
                    return kmers[rowIndex].sequence();
                case 1:
                    return kmers[rowIndex].count() * 5;
                case 2:
                    return kmers[rowIndex].pValue();
                case 3:
                    return kmers[rowIndex].maxObsExp();
                case 4:
                    return groups[kmers[rowIndex].maxPosition() - 1].toString();
