...
public class LogarithmicPercentLineGraph extends JPanel {
    ...
    public LogarithmicPercentLineGraph(double[][] data, double minY, double maxY, String xLabel, String[] xTitles, int[] xCategories, String graphTitle) {
    ...
    public LogarithmicPercentLineGraph(double[][] data, double minY, double maxY, String xLabel, String[] xTitles, String[] xCategories, String graphTitle) {
        this.data = data;
        this.minY = minY;
        this.maxY = maxY;
...
//        System.out.println(findOptimalYInterval(maxY));
    }
...
//            label = label.replaceAll(".0$", "");
            ...
            g2.drawString(label, 37, getY_Data(y) + (g.getFontMetrics().getAscent() / 2));
        }
        xOffset += 40;
        // Draw the graph title
        g2.setFont(Statics.Graph_Max_Font);
        g2.drawString(graphTitle, 50, getY_Data(maxY) - 25 + (g2.getFontMetrics().getAscent() / 2));
        ...
        for (int i = 0; i <= 7; i += 1) {
            ...
            g2.drawLine(xOffset, getY_Data(y), data[0].length * baseWidth + xOffset, getY_Data(y));
        }
        g2.setColor(Color.BLACK);
        // Now draw the axes
        ...
        for (int d = 0; d < data.length; d++) {
            ...
            lastY = getY_Data(data[d][0]);
            for (int i = 1; i < data[d].length; i++) {
                int thisY = getY_Data(data[d][i]);
        ...
        g2.drawString("Percent", 1, (getY_Data(maxY - yStart)));
        g2.rotate(-Math.toRadians(-90));
        g2.translate(-(float) -13, -(float) 300);
    }

    private int getY_Data(double y) {
