...
public class ReadLengthBarChart extends JPanel {
    ...
    public ReadLengthBarChart(double[][] data, double minY, double maxY, String xLabel, String[] xTitles, int[] xCategories, String graphTitle) {
    ...
    public ReadLengthBarChart(double[][] data, double minY, double maxY, String xLabel, String[] xTitles, String[] xCategories, String graphTitle) {
        this.data = data;
        this.minY = minY;
        this.maxY = maxY;
        this.xLabel = xLabel;
    ...
    public void paint(Graphics g) {
        ...
        g.drawLine(xOffset, getHeight() - 40, data[0].length * baseWidth + xOffset, getHeight() - 40);
        g.drawLine(xOffset, getHeight() - 40, xOffset, 40);
        // Now draw the datasets
        for (int i = 0; i < data[0].length; i++) {
            String[] xCat = xCategories[i].split("-");
            int xLabelg;
            if (xCat.length == 1) {
                xLabelg = Integer.parseInt(xCat[0]);
            } else {
                xLabelg = Integer.parseInt(xCat[1]);
            }
            if (xLabelg < 20) {
